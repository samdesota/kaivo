...
export class SqliteRealtimeEngine {
  ...
  private drain(): void {
    ...
    if (this.closed || (this.sqlite as unknown as { open?: boolean }).open === false) return
...
function rowToEvent(row: unknown): SqliteRealtimeEvent {
  ...
  return { seq: r.seq, table: r.table_name, op: r.op, key: r.row_key, row: r.row_json ? JSON.parse(r.row_json) as Record<string, unknown> : null, committedAt: r.committed_at }
}
