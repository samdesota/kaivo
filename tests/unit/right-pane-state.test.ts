...
describe('rightPaneReducer', () => {
  ...
  it('starts with one newtab that is active', () => {
    expect(state.tabs).toHaveLength(1)
    expect(state.tabs[0]?.content.type).toBe('newtab')
  ...
  it('opens a shell tab and activates it by default', () => {
    ...
    const shell = s1.tabs.find((t) => t.content.type === 'shell')!
  ...
  it('reuses an existing tab when opening the same shell again', () => {
    ...
    const s2 = rightPaneReducer(s1, { type: 'activate', tabId: state.activeTabId })
    const s3 = open(s2, { type: 'shell', shellId: 'abc' })
    expect(s3.tabs).toHaveLength(2)
    expect(s3.activeTabId).toBe(s1.tabs.find((t) => t.content.type === 'shell')!.id)
  })
  ...
  it('file and preview tabs dedupe by path and port', () => {
    ...
    expect(kinds).toEqual(['file', 'newtab', 'preview'])
  })
  ...
  it('closing the last tab leaves a fresh newtab', () => {
    const id = state.tabs[0]!.id
    const s = rightPaneReducer(state, { type: 'close', tabId: id })
    expect(s.tabs).toHaveLength(1)
    expect(s.tabs[0]?.content.type).toBe('newtab')
    expect(s.activeTabId).toBe(s.tabs[0]?.id)
  })

  it('setTitle updates the named tab only', () => {
    const s1 = open(state, { type: 'shell', shellId: 'x' })
    const shellTab = s1.tabs.find((t) => t.content.type === 'shell')!
    const s2 = rightPaneReducer(s1, { type: 'setTitle', tabId: shellTab.id, title: 'build' })
    expect(s2.tabs.find((t) => t.id === shellTab.id)?.title).toBe('build')
    expect(s2.tabs.find((t) => t.content.type === 'newtab')?.title).toBe('New')
  })
  ...
  it('hydrate falls back if activeTabId is missing from stored tabs', () => {
    const stored: RightPaneState = {
      tabs: [{ id: 'x', title: 'x', content: { type: 'newtab' } }],
  ...
  it('hydrate of empty state returns the default initial state', () => {
