...
async function promptOnce(
  ...
  const promptPromise = client.session.promptAsync({
    ...
    body: {
      ...
      tools: opts.tools ?? { bash: false, kaivo_bash: true, kaivo_pty: true },
      model: MODEL,
    },
    throwOnError: true,
  })
...
const cases: CaseDef[] = [
  {
    ...
    async run(ctx) {
      const r = await promptOnce(
        ...
        { tools: { bash: false, kaivo_bash: false, kaivo_pty: false } },
      )
  ...
  {
    name: 'tool: kaivo_bash echoes a marker',
    async run(ctx) {
      const marker = `E2E_${Date.now().toString(36).toUpperCase()}`
      const r = await promptOnce(
        ctx,
        `Run the shell command: echo ${marker}. Use kaivo_bash.`,
      )
      const tool = r.parts.find(
        (p) => p.type === 'tool' && p.tool === 'kaivo_bash' && p.state?.status === 'completed',
      )
      if (!tool) throw new Error('no completed kaivo_bash tool part in transcript')
