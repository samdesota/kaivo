...
- Kaivo plugin: the container image ships it at `/opt/kaivo-opencode-plugin/index.js`; the npm-bundled local variant ships it inside the `cc-env` install (resolve via `require.resolve` to give opencode an absolute `file://` URL).
