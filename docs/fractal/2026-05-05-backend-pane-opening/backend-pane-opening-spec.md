...
`cloud_open_pane` should open Cloud Code panes through backend state instead of depending on a currently rendered frontend event handler. Opening a file, shell, preview, or browser pane must work even when no frontend is mounted at the moment the tool is called.
