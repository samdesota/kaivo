...
export function ShellsScope(props: UniversalScopeProps) {
  ...
  const results = useMemo(() => shellScopeResults({ shells: shells.data as ShellRow[] | undefined, loading: shells.isLoading, error: shells.error, query, homePath, openShell: (shellId) => onOpenContent?.({ type: 'shell', shellId }) }), [homePath, onOpenContent, query, shells.data, shells.error, shells.isLoading])
  ...
  return <UniversalMenuResultList results={results} activeIndex={activeIndex} mouseMoved={mouseMoved} onMouseMoved={onMouseMoved} onActiveChange={onActiveChange} onSelect={(index, event) => void selectResult(results, index, onClose, event)} loading={shells.isFetching} />
}
