...
/** Read/write the open flag for a row. `defaultOpen` is the initial value. */
export function useOpenState(
  id: string,
  defaultOpen: boolean,
): [boolean, (v: boolean | ((prev: boolean) => boolean)) => void] {
  // When there's no provider (e.g. nested SubagentTranscript outside the
  // outer virtualizer's tree, tests), fall back to a per-row store so each
  // row still toggles independently.
