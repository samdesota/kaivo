...
/**
 * Renders the agent's pending question(s). One request can carry multiple
 * sub-questions; we render each one and gather a parallel array of answers
 * to send back via agent.sessionAnswerQuestion.
 */
export function QuestionBanner({
  ...
  const reply = trpc.agent.sessionAnswerQuestion.useMutation()
  const reject = trpc.agent.sessionRejectQuestion.useMutation()
...
interface QuestionDraft {
  /** Selected option labels (for choice-style questions). */
  selected: Set<string>
  /** Free-text answer (for `custom` questions or no-options questions). */
  custom: string
}

function initialDraft(q: QuestionItem): QuestionDraft {
